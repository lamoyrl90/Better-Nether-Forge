...
public class StructureNBT 
	{
		...
		public StructureNBT(String template)
		{
			location = new ResourceLocation(BetterNether.MODID, template);
			this.template = readStructureFromJar(location);
		}
		...
		public boolean generateCentered(IWorld world, BlockPos pos)
		{
			...
			template.func_237144_a_(world, pos.add(-blockpos2.getX() >> 1, 0, -blockpos2.getZ() >> 1), data, world.getRandom());
			return true;
		}
		...
		{
			return template.getMutableBoundingBox(new PlacementSettings().setRotation(this.rotation).setMirror(mirror), pos);
		}
	}