...
public class BuildingStructureProcessor extends StructureProcessor {
	...
	public BlockInfo process(IWorldReader worldView, BlockPos pos, BlockPos blockPos, BlockInfo structureBlockInfo, BlockInfo structureBlockInfo2, PlacementSettings structurePlacementData, Template template)
	{
		...
		else if (name.contains("plank") || name.contains("reed"))
		{
			if (block instanceof StairsBlock)
			{
		...
		else if (block instanceof SmallCincinnasiteLanternBlock)
		{
			if (state.get(SmallCincinnasiteLanternBlock.FACING) == Direction.UP)
				return setState(palette.getCeilingLight(state), structureBlockInfo2);
			else if (state.get(SmallCincinnasiteLanternBlock.FACING) != Direction.DOWN)
		...
		else if (block instanceof BNPotBlock)
		{
			return setState(palette.getPot(state), structureBlockInfo2);
		}
		else if (block instanceof PottedPlantBlock)
		{
		...
		else if (!name.contains("nether") && !name.contains("mycelium") && state.isOpaqueCube(worldView, structureBlockInfo.pos) && state.isSolid() && !(state.getBlock() instanceof ContainerBlock))
		{
