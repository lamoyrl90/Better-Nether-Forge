...
public class BoneStalactiteFeature extends Feature<NoFeatureConfig> {
	...
	public BoneStalactiteFeature(Codec<NoFeatureConfig> codec) {
		super(codec);
	}
	...
	public void generate(IWorld world, BlockPos pos, Random random)
	{
		if (!invalidPosition(world, pos))
		{
			for (int i = 0; i < 16; i++)
			{
				...
				if (!invalidPosition(world, POS))
				{
					upper=BNBlocks.BONE_BLOCK.get();
					block=BNBlocks.BONE_SPELEOTHEM.get();
					int dx = x - pos.getX();
					int dz = z - pos.getZ();
