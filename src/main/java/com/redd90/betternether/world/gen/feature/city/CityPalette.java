...
public class CityPalette {
	...
	{
		if (source.getBlock() instanceof SmallCincinnasiteLanternBlock && !(block instanceof SmallCincinnasiteLanternBlock))
		{
			Direction facing = source.get(SmallCincinnasiteLanternBlock.FACING);
			BlockState state = block.getDefaultState();
	...
	public BlockState getPlant(BlockState input)
	{
		...
		RANDOM.setSeed(seed.hashCode());
		return BNBlocks.POTTED_PLANT.get().getDefaultState().with(PottedPlantBlock.PLANT, PottedPlantShape.values()[RANDOM.nextInt(PottedPlantShape.values().length)]);
	}
}
