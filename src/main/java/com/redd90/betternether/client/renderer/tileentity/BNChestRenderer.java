package com.redd90.betternether.client.renderer.tileentity;
...
public class BNChestRenderer extends TileEntityRenderer<BNChestTileEntity> {
	private static final HashMap<String, RenderType[]> LAYERS = Maps.newHashMap();
	...
	public BNChestRenderer(TileEntityRendererDispatcher tileEntityRenderDispatcher)
	{
		super(tileEntityRenderDispatcher);
	...
	public void render(BNChestTileEntity entity, float tickDelta, MatrixStack matrices, IRenderTypeBuffer vertexConsumers, int light, int overlay)
	{
		...
		if (block instanceof AbstractChestBlock)
		{
			...
			RenderType[] layers = LAYERS.get(entity.getMaterial());
			RenderType layer = getChestTexture(chestType, layers);
			IVertexBuilder vertexConsumer = vertexConsumers.getBuffer(layer);
	...
	static
	{
		Block[] chests = BNBlocks.getChests();
		
		for (Block chest : chests) {
			String name = chest.getRegistryName().getPath();
			LAYERS.put(name, new RenderType[] {
					RenderType.getEntitySolid(new ResourceLocation(BetterNether.MODID, "textures/entity/chest/" + name + ".png")),
					RenderType.getEntitySolid(new ResourceLocation(BetterNether.MODID, "textures/entity/chest/" + name + "_left.png")),
					RenderType.getEntitySolid(new ResourceLocation(BetterNether.MODID, "textures/entity/chest/" + name + "_right.png"))
			});
			
		}
		LAYERS.put("normal", new RenderType[] {
