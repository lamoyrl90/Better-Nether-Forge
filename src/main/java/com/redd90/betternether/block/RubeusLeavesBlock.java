...
public class RubeusLeavesBlock extends BNBlock {
	...
	private static BlockState updateDistanceFromLogs(BlockState state, IWorld world, BlockPos pos)
	{
		...
		for (int n = 0; n < count; ++n)
		{
			...
			mutable.func_239622_a_(pos, dir);
	...
	{
		if (state.getBlock().isIn(BNTags.Blocks.RUBEUS_LOGS))
		{
			return 0;
		}
		else
		{
			return state.getBlock() instanceof RubeusLeavesBlock ? state.get(DISTANCE_CUSTOM) : MAX_DIST;
		}
	}
